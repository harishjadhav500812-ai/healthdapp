...
const App = () => {
  ...
  // Monitor user state changes and redirect
  useEffect(() => {
    ...
    if (user) {
      // User is logged in, check if we should redirect
      if (currentPath.includes("/login") || currentPath.includes("/signup")) {
        const userRole = user.role;
        console.log("User logged in with role:", userRole);

        if (userRole === "PATIENT" || userRole === UserRole.PATIENT) {
          console.log("🔄 Redirecting to PATIENT dashboard");
          setCurrentPath("/patient/dashboard");
        } else if (userRole === "DOCTOR" || userRole === UserRole.DOCTOR) {
  ...
  const handleLogin = (userData: User) => {
    ...
    // Just set the user - useEffect will handle redirect
    setUser(userData);
  };
